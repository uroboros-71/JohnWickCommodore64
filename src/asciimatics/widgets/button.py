...
class Button(Widget):
    ...
    __slots__ = ["_text", "_text_raw", "_add_box", "_on_click", "_reverse", "_align"]

    def __init__(self, text, on_click, label=None, add_box=True, name=None, reverse=False, align="^", **kwargs):
        """
        ...
        if self._add_box:
            ...
            if offset == 0:
                if self._align == "<":
                    ...
                    self._x += max(0, (self.width - text_width))
                else:
    ...
    def update(self, frame_no):
        ...
        if self._reverse:
            (colour, attr, bg) = self._pick_colours("button_reverse")
        else:

...
from asciimatics.widgets.utilities import _split_text, _enforce_width
...
class Label(Widget):
    ...
    __slots__ = ["_text", "_required_height", "_align", "_attr", "_start_column"]

    def __init__(self, label, height=1, align="<", name=None, attr=False):
        """
    ...
    def update(self, frame_no):
        if self._attr :
            ...
            self._pick_palette_key("label_reverse", selected=False, allow_input_state=False)]
            text = self._text + self._value[self._start_column:]
            #text = _enforce_width(text, self.width, self._frame.canvas.unicode_aware)
            self._frame.canvas.paint(
                f"{text:{self._align}{self._w}}",
                self._x, self._y, colour, attr, background
            )
        else:
            (colour, attr, background) = self._frame.palette[

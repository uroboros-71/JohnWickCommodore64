...
class Text(Widget):
    ...
    __slots__ = ["_column", "_start_column", "_on_change", "_validator", "_hide_char", "_max_length", "_attr", "_force_x"]

    def __init__(self, label=None, name=None, on_change=None, validator=None, hide_char=None,
                 max_length=None, readonly=False, attr=False, force_x=0, **kwargs):
        """
    ...
    def update(self, frame_no):
        self._draw_label(self._force_x)
        ...
        if self._attr :
            (colour, attr, background) = self._pick_colours("readonly" if self._readonly else "edit_text_reverse")
        else:
        ...
        self._frame.canvas.print_at(
            text,
            self._x + self._offset + self._force_x,
            self._y,
        ...
        if isinstance(event, KeyboardEvent):
            if event.key_code == Screen.KEY_BACK and not self._readonly:
                ...
                if self._max_length is None or len(self._value) < self._max_length:
                    ...
                    self._set_value(chr(event.key_code).join([self._value[:self._column],
                                                                        self._value[self._column:]]))
                    self._column += 1
                    if self._max_length == len(self._value):
                        self._check_value()
            else:
    ...
    def value(self, new_value):
        self._set_value(new_value, reset=True)

    def _set_value(self, new_value, reset=False):
